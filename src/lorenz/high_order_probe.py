...
def validate_probe_config(config: dict) -> dict:
    ...
    if mixed_config is not None:
        ...
        if configured_by_direction is None:
            ...
            if not isinstance(configured_by_direction, dict):
                raise ValueError("strengths_by_direction must map labels to strengths")
            if set(configured_by_direction) != set(direction_labels):
                raise ValueError("strengths_by_direction keys must match direction labels")
            strengths_by_direction = [
                validate_strengths(configured_by_direction[label], minimum_count=1)
                for label in direction_labels
            ]
            if not np.array_equal(np.unique(np.concatenate(strengths_by_direction)), strengths):
                raise ValueError("strengths must be the union of strengths_by_direction")
    ...
    if (configured_by_direction is not None or mixed_mode) and config.get("extension") is not None:
        raise ValueError("extension runs do not support per-direction or mixed designs")
...
def run_probe(config_path, resume_dir=None):
    ...
    derived = {
        ...
        "strengths_by_direction": config.get("strengths_by_direction"),
